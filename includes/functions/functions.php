...
// CHECK REG_STATUS  for user If function return 1 user unactivated if 0 user active
function checkUserStatus($user_name)
{
    global $conn;

    $stm = $conn->prepare("
         SELECT user_name,reg_status FROM users WHERE user_name=? AND reg_status=0
    ");
...
function checkItemStatus($item_id)
{
    global $conn;
    $stm = $conn->prepare("SELECT item_id,is_item_approved FROM items WHERE item_id=? AND is_item_approved=1");
    $stm->execute([$item_id]);
    $status=$stm->rowCount();
    return $status;
}
